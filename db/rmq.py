...
class RabbitMQClient:
    ...
    async def start_consuming(self, queue_name:str, on_message_callback:Callable[[aio_pika.IncomingMessage], Coroutine[Any,Any, None]]):
        """
    ...
    async def rpc_call(self, queue_name:str, message_body:Dict[str,Any], timeout:int=20)->Dict[str,Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=408,
                detail="Grading request time out. Submission is being processed in the background"
            )
